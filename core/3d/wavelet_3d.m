...
%       resolution (int): the log of spatial resolution
...
function [y_Phi, y_Psi, meta_Phi, meta_Psi] = wavelet_3d(y, filters, filters_rot, options)
    ...
    white_list = {'resolution', 'psi_mask', 'oversampling', 'oversampling_rot' , 'precision' };
    ...
    sz_paded = filters.phi.filter.N / 2^(options.resolution);
    ...
    if (~calculate_psi && 2^ds_angle == size(y,3))
        ...
        % low pass spatial
        downsampling_rate = max(floor(J/Q)- options.resolution - options.oversampling, 0);
        tmp = fft2(pad_signal(y_phi_angle, sz_paded, []));
        tmp = conv_sub_2d(tmp, filters.phi.filter, downsampling_rate);
        y_Phi = real(unpad_signal(tmp, downsampling_rate*[1 1],  [size(y,1), size(y,2)]));
        %meta
        meta_Phi.J(1) = filters.phi.meta.J;
        meta_Phi.resolution(1) = options.resolution+downsampling_rate;
    else
        ...
        for theta = 1:nb_angle_in
            downsampling_rate = max(floor(J/Q) - options.resolution - options.oversampling, 0);
            tmp = ...
                real(conv_sub_2d(yf(:,:,theta), filters.phi.filter, downsampling_rate));
            tmp = unpad_signal(tmp, downsampling_rate*[1 1], [size(y,1), size(y,2)]);
        ...
        downsampling_rate_rot = floor(max(floor(J_rot/Q_rot) - options.oversampling_rot, 0));
        if (2^downsampling_rate_rot == size(y_phi,3)) % if there is one coefft left, compute the sum
            % is faster than convolving with a constant filter
            y_Phi = sum(y_phi,3) / 2^(downsampling_rate_rot/2);
        else
            % fourier angle
            y_phi_f = fft(y_phi, [], 3);
            y_Phi = real(...
                sub_conv_1d_along_third_dim_simple(y_phi_f, phi_angle, downsampling_rate_rot));
        end
        meta_Phi.J(1) = filters.phi.meta.J;
        meta_Phi.resolution(1) = options.resolution+downsampling_rate;
    end
    ...
    if (calculate_psi)
        ...
        for lambda2 = find(options.psi_mask)
            ...
            downsampling_rate = max(floor(j2/Q)- options.resolution - options.oversampling, 0);
            
            % convolution with psi_{j1, theta + theta2}
            for theta = 1:nb_angle_in
                theta_sum_mod2L =  1 + mod(theta + theta2 - 2, 2*L);
                theta_sum_modL =  1 + mod(theta + theta2 - 2, L);
                lambda2p1 = filters.psi.meta.theta == theta_sum_modL & ...
                    filters.psi.meta.j == j2;
                psi = filters.psi.filter{lambda2p1};
                
                tmp = conv_sub_2d(yf(:,:,theta), psi, downsampling_rate);
                tmp = unpad_signal(tmp, downsampling_rate*[1,1], [size(y,1), size(y,2)]);
            ...
            meta_Psi.k2(p) = -1;
            meta_Psi.resolution(p) = options.resolution+downsampling_rate;
            p = p+1;
